...
{{
    ...
    "occasion": "Dịp sử dụng (ví dụ: đi làm, dự tiệc, đi chơi, ở nhá)",
...
# ============================================
# HANDLE TEXT - XỬ LÝ VỚI FUNCTION CALLING (ĐÃ SỬA LỖI 1)
# ============================================
...
def handle_text(uid: str, text: str):
    ...
    try:
        ...
        # ============================================
        # QUAN TRỌNG: XỬ LÝ TIN NHẮN ĐẦU TIÊN SAU REFERRAL
        # ============================================
        ...
        if pending_ms and pending_ms in PRODUCTS and first_message:
            ...
            # QUAN TRỌNG: XÓA TẤT CẢ TRẠNG THÁI PENDING NGAY LẬP TỨC
            ctx["pending_carousel_ms"] = None
            ctx["first_message_after_referral"] = False
            
            # QUAN TRỌNG: KHÔNG CHẠY TIẾP, TRÁNH XỬ LÝ SONG SONG
...
# ============================================
# IMPROVED: XỬ LÝ POSTBACK VỚI RECOVERY MECHANISM (ĐÃ SỬA LỖI 2)
# ============================================
...
def handle_postback_with_recovery(uid: str, payload: str, postback_id: str = None):
    ...
    if postback_id:
        ...
        # Nếu đã xử lý trong 30 giây gần đây → bỏ qua
    ...
    # 3. SET LOCK VỚI TIMEOUT (tránh deadlock) - THÊM KIỂM TRA TIME
    if ctx.get("processing_lock"):
        lock_time = ctx.get("processing_lock_time", 0)
        # Nếu lock quá 15 giây → force release và ghi log
        if now - lock_time > 15:
            print(f"[LOCK RECOVERY] ⚠️ FORCE RELEASE LOCK sau 15s cho user {uid}")
            ctx["processing_lock"] = False
        else:
            print(f"[POSTBACK LOCKED] User {uid} đang được xử lý, bỏ qua")
            return False
    
    # 4. XỬ LÝ POSTBACK VỚI TRY-EXCEPT-FINALLY
    ctx["processing_lock"] = True
    ...
    try:
        ...
        if payload.startswith("ADVICE_"):
            ...
            if ms in PRODUCTS:
                # KIỂM TRA DEBOUNCE NÂNG CAO CHO ADVICE
                last_info_ms = ctx.get("product_info_sent_ms")
                last_info_time = ctx.get("last_product_info_time", 0)
                
                if last_info_ms == ms and (now - last_info_time) < 15:
                    print(f"[ADVICE DEBOUNCE] Đã gửi sản phẩm {ms} trong 15s, bỏ qua")
                    return True
            ...
            if ms in PRODUCTS:
                # KIỂM TRA DEBOUNCE CHO XEM ẢNH
                last_image_time = ctx.get("last_all_images_time", 0)
                if now - last_image_time < 10:
                    print(f"[VIEW IMAGES DEBOUNCE] Đã gửi ảnh trong 10s, bỏ qua")
                    return True
...
def webhook():
    ...
    for e in entry:
        ...
        for m in messaging:
            ...
            # ============================================
            # KIỂM TRA VÀ RESET LOCK NẾU QUÁ LÂU (15s)
            # ============================================
            ...
            if ctx.get("processing_lock"):
                lock_time = ctx.get("processing_lock_time", 0)
                if now - lock_time > 15:
                    print(f"[WEBHOOK LOCK RECOVERY] ⚠️ Force reset lock cho user {sender_id}, lock đã {now-lock_time:.1f}s")
                    ctx["processing_lock"] = False
...
def health_check():
    ...
    return {
        ...
        "postback_double_processing_fix": "ENABLED (atomic lock + strict duplicate detection)",
        "product_info_debounce": "10s cho cùng sản phẩm, 3s cho bất kỳ sản phẩm",
        "lock_recovery_mechanism": "ENABLED (auto release sau 15s)",
        "postback_duplicate_protection": "ENABLED (30s memory)",
        "fixed_issue_1": "Tin nhắn đầu tiên chỉ gửi carousel, không chạy function calling",
        "fixed_issue_2": "Postback deadlock fixed with 15s timeout and recovery"
    }, 200
...
if __name__ == "__main__":
    ...
    print(f"🔴 POSTBACK DOUBLE PROCESSING FIX: ĐÃ SỬA - Atomic lock + Strict duplicate detection")
    ...
    print(f"🔴 Postback Duplicate Protection: 30s memory")
    print(f"🔴 Fixed Issue 1: Tin nhắn đầu tiên chỉ gửi carousel, không chạy function calling")
    print(f"🔴 Fixed Issue 2: Postback deadlock fixed with 15s timeout and recovery")
