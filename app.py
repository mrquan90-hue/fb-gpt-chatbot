...
def handle_text(uid: str, text: str):
    ...
    try:
        ...
        # ============================================
        # QUY TẮC QUAN TRỌNG: XỬ LÝ TIN NHẮN ĐẦU TIÊN SAU REFERRAL
        # ============================================
        referral_source = ctx.get("referral_source")
        pending_ms = ctx.get("pending_carousel_ms")
        
        # ▼▼▼ ĐIỀU KIỆN QUAN TRỌNG: Nếu có pending_ms và referral_source hợp lệ
        valid_referral_sources = ["fchat_echo", "ADS", "catalog", "GET_STARTED"]
        
        if pending_ms and referral_source in valid_referral_sources:
            # KIỂM TRA: Đây có phải tin nhắn đầu tiên sau referral không?
            has_sent_carousel = ctx.get("has_sent_first_carousel", False)
            
            if not has_sent_carousel and pending_ms in PRODUCTS:
                print(f"[FIRST MESSAGE RULE] Bỏ qua tin nhắn đầu tiên '{text[:30]}...' và gửi carousel {pending_ms}")
                ...
                # 3. KHÔNG XỬ LÝ TIN NHẮN NÀY BẰNG GPT
                ctx["processing_lock"] = False
                return
        
        # ============================================
        # TỪ TIN NHẮN THỨ 2 TRỞ ĐI: XỬ LÝ BÌNH THƯỜNG
        # ============================================
        ...
        for m in messaging:
            ...
            if m.get("message", {}).get("is_echo"):
                ...
                if detected_ms and detected_ms in PRODUCTS:
                    ...
                    try:
                        # **QUAN TRỌNG: CHỈ CẬP NHẬT CONTEXT, KHÔNG GỬI GÌ CẢ**
                        ctx["last_ms"] = detected_ms
